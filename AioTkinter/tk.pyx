...
class Bindings(enum.StrEnum):
    ...
    @staticmethod
    cpdef bool IsDigit(Event event):
        """
        :param event:
    ...
    @staticmethod
    cpdef Bindings FromEvent(Event event):
        """
...
cpdef class Event(object):
    ...
    def __cinit__(self, str keycode, int num, int height =-1, int width, int key_code, int state, int  x, int y, str char, str keysym, int keysym_num, int delta,
                  float x_root = 0, float y_root = 0, EventType event_type, Widget widget, float time, bool focus):
        self.num = num
        self.width = width
        self.height = height
        self.state = state
    ...
    @classmethod
    cpdef Event Create(cls, tkRoot root, int num = -1, int height =-1, int width = -1, int key_code = -1, int state = -1, int  x = -1, int y = -1, str keycode = '??',
                       str keysym = '??', int keysym_num = -1, int delta = -1, EventType event_type, Widget widget, current_time=time()):
    ...
    def __str__(self) -> str:
        return f"""<TkinterEvent(). {pprint.pformat(self.ToDict(), indent=4)} >"""

    def ToDict(self):
        return {
            'KeySynonym': self.KeySynonym, 'IsDigit': self.IsDigit(self.KeySynonym), 'IsEnter': self.IsEnter(), 'IsValid': self.IsValid(),  # 'serial':     self.serial,
            'num':        self.num, 'Height': self.height, 'keycode': self.keycode, 'state': self.state, 'time': self.time, 'Width': self.width, 'x': self.x, 'y': self.y,
            'char':       self.character, 'keysym': self.keysym, 'keysym_num': self.keysym_num, 'type': self.type, 'widget': self.widget, 'x_root': self.x_root,
            'y_root':     self.y_root, 'delta': self.delta, 'send_event': self.send_event
            }
    ...
    def __repr__(self):
        ...
        if not self.character:
            ...
            attrs['char'] = repr(self.character)
    ...
    @staticmethod
    cpdef bool IsDigit(Event keysym):
        """
        :param keysym:
    ...
    cpdef EventType EventType(self):
        return self.type
    ...
    @classmethod
    cpdef Event FromShiftTabEvent(cls, Event event):
        e = cls.__new__(event)
...
def _get_ints(tk: _tkinter.TkappType, string: str) -> Optional[Tuple[int, ...]]:
    """Internal function."""
    if string:
        return tuple(map(tk.getint, tk.splitlist(string)))

def _get_doubles(tk: _tkinter.TkappType, string: str)-> Optional[Tuple[float, ...]]:
    """Internal function."""
    if string:
        return tuple(map(tk.getdouble, tk.splitlist(string)))

def _get_boolean(tk: _tkinter.TkappType, string: str) -> Optional[bool]:
    """Internal function."""
    if string:
        return tk.getboolean(string)
...
class Misc:
    ...
    def winfo_children(self):
        ...
        for child in self.tk.splitlist(self.tk.call('winfo', 'children', self._w)):
            try:
                # Tcl sometimes returns extra windows, e.g. for menus; those need to be skipped
    ...
    def winfo_geometry(self):
        """Return geometry string for this widget in the form "width x height + X + Y"."""
    ...
    def winfo_pointerxy(self):
        ...
        return _get_ints(self.tk.call('winfo', 'pointerxy', self._w))
    ...
    def grid_bbox(self, column=None, row=None, col2=None, row2=None):
        ...
        if col2 is not None and row2 is not None:
            args = args + (col2, row2)
        return _get_ints(self.tk.call(*args)) or None
    ...
    def grid_location(self, x, y):
        ...
        widget is located."""
        return _get_ints(self.tk.call('grid', 'location', self._w, x, y)) or None
    ...
    def grid_size(self):
        ...
        return _get_ints(self.tk.call('grid', 'size', self._w)) or None
...
class CallWrapper(object):
    ...
    def __init__(self, func: Callable, subst, widget):
...
class Wm:
    ...
    def wm_aspect(self, minNumer=None, minDenom=None, maxNumer=None, maxDenom=None):
        ...
        return _get_ints(self.tk.call('wm', 'aspect', self._w, minNumer, minDenom, maxNumer, maxDenom))
    ...
    def wm_grid(self, baseWidth=None, baseHeight=None, widthInc=None, heightInc=None):
        ...
        return _get_ints(self.tk.call('wm', 'grid', self._w, baseWidth, baseHeight, widthInc, heightInc))
    ...
    def wm_iconposition(self, x=None, y=None):
        ...
        return _get_ints(self.tk.call('wm', 'iconposition', self._w, x, y))
    ...
    def wm_maxsize(self, width=None, height=None):
        ...
        is given."""
        return _get_ints(self.tk.call('wm', 'maxsize', self._w, width, height))
    ...
    def wm_minsize(self, width=None, height=None):
        ...
        the values are given in grid units. Return the current values if None
        is given."""
        return _get_ints(self.tk.call('wm', 'minsize', self._w, width, height))
...
class BaseWidget(Misc):
    """Internal class."""
    children: Dict[str, BaseWidget]
    tk: _tkinter.TkappType
    def _setup(self, master: BaseWidget, cnf):
...
class tkRoot(Tk, _rootMixin):
    ...
    def Create_Event(self, tag: Union[str, Bindings], *, num: int = '??', height: int = '??', width: int = '??', key_code: int = '??', state: int = '??', x: int = '??',
                     y: int = '??', char: str = '??', keysym: Union[str, Bindings] = '??', keysym_num: int = '??', delta: int = '??', event_type: EventType, widget: Widget,
    ...
    def do_one_event(self):
        return self.tk.dooneevent(_tkinter.DONT_WAIT)
class tkTopLevel(Toplevel, _rootMixin):
    __slots__ = ['style']
...
class Canvas(Widget, XView, YView):
    ...
    def bbox(self, *args):
        ...
        return _get_ints(self.tk.call((self._w, 'bbox') + args)) or None
    ...
    def find(self, *args):
        """Internal function."""
        return _get_ints(self.tk.call((self._w, 'find') + args)) or ()
...
class Listbox(Widget, XView, YView):
    ...
    def bbox(self, index):
        ...
        return _get_ints(self.tk.call(self._w, 'bbox', index)) or None

    def curselection(self):
        """Return the indices of currently selected item."""
        return _get_ints(self.tk.call(self._w, 'curselection')) or ()
...
class Scrollbar(Widget):
    ...
    def get(self):
        ...
        return _get_doubles(self.tk.call(self._w, 'get'))
...
class Text(Widget, XView, YView):
    ...
    def dlineinfo(self, index):
        ...
        return _get_ints(self.tk.call(self._w, 'dlineinfo', index))
...
class OptionMenu(Menubutton):
    ...
    def __init__(self, master, variable, value, *values, **kwargs):
        ...
        kw = {
            "borderwidth": 2, "textvariable": variable, "indicatoron": 1, "relief": RAISED, "anchor": "c", "highlightthickness": 2
            }
...
class Spinbox(Widget, XView):
    ...
    def scan(self, *args):
        """Internal function."""
        return _get_ints(self.tk.call((self._w, 'scan') + args)) or ()
    ...
    def selection(self, *args):
        """Internal function."""
        return _get_ints(self.tk.call((self._w, 'selection') + args)) or ()
...
class PanedWindow(Widget):
    ...
    def proxy(self, *args):
        """Internal function."""
        return _get_ints(self.tk.call((self._w, 'proxy') + args)) or ()
    ...
    def sash(self, *args):
        """Internal function."""
        return _get_ints(self.tk.call((self._w, 'sash') + args)) or ()
...
__all__ = [name for name, obj in globals().items() if not name.startswith('_') and not isinstance(obj, types.ModuleType) and name not in { 'wantobjects' }.union(dir(_typing))]
